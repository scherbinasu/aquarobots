try:
    ...
    def rotateToColor(color, speed=30, reverse=False, duo=0):
        while True:
            contours, img = getContoursColor(color, cam.get_frame)
    ...
    # out.release()
    # video_sleep(2, out, FPS, lambda: cam.get_frame())
    run_go_to_ports(motor_left, motor_right, lambda: cam.get_frame(), out, FPS)
    ...
    while 1:
        ...
        corners, ids, rejected = aruco  # распаковываем кортеж
        if ids is None or len(ids) == 0 or ids[0] != 239:
            video_sleep(0.1, out, FPS, lambda: cam.get_frame())
            continue
